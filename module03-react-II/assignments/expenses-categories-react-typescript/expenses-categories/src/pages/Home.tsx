...
export function Home() {
    ...
    const alimentacao = expenses.filter((item) => { return item.categoria === "Alimentação"}).reduce((acc, curr) => {
        return acc + curr.valor;
    }, 0);

    const saude = expenses.filter((item) => { return item.categoria === "Saúde" }).reduce((acc, curr) => {
        return acc + curr.valor;
    }, 0);
